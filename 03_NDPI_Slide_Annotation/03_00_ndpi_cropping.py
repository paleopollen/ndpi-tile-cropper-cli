...
def crop_image(i, start_xy_list, zPlane, crops_dir, img_path, width, height):
    start_x = start_xy_list[i][0]
    start_y = start_xy_list[i][1]
    print("Processing coordinates: ", start_y, start_y)
...
with multiprocessing.Pool(processes=4) as pool:
    results = [pool.apply(crop_image, args=(i, start_xy_list, zPlane, crops_dir, img_path, width, height)) for i in
               index_list]
